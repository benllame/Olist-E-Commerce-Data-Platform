...
class OrderEventSimulator:
    ...
    def generate_order_event(self):
        ...
        event = {
            ...
            "metadata": {"source": "simulator", "version": "1.0"},
        }
    ...
    def publish_event(self, event):
        ...
        try:
            ...
            logging.info(
                f"Published: {event['event_type']} | Order: {event['order_id']} | ID: {message_id}"
            )
            return message_id
    ...
    def run(self, events_per_second=1, duration_seconds=None):
        ...
        logging.info(
            f"Duration: {'infinite' if duration_seconds is None else f'{duration_seconds}s'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--topic", type=str, default=TOPIC_NAME, help="Pub/Sub topic name"
    )
    parser.add_argument(
        "--rate", type=float, default=1.0, help="Events per second (default: 1.0)"
    )
    parser.add_argument(
        "--duration",
        type=int,
    ...
    # Ejecutar
    simulator.run(events_per_second=args.rate, duration_seconds=args.duration)
