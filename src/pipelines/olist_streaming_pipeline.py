...
class EnrichEvent(beam.DoFn):
    ...
    def process(self, element):
        try:
            ...
            # Hora del día
            timestamp = datetime.fromisoformat(enriched['event_timestamp'].replace('Z', '+00:00'))
            hour = timestamp.hour
            if 6 <= hour < 12:
...
def run_streaming_pipeline(options):
    ...
    with beam.Pipeline(options=options) as pipeline:
        ...
        sliding_metrics = (
            enriched_events
            | 'Sliding Window 5min' >> beam.WindowInto(
                SlidingWindows(300, 60)  # 5min window, 1min slide
            )
        ...
        if options.view_as(StandardOptions).runner == 'DirectRunner':
            (
                sliding_metrics
                | 'Log Rolling Metrics' >> beam.Map(
                    lambda x: logging.info(f"Rolling metrics: {x}")
                )
            )
