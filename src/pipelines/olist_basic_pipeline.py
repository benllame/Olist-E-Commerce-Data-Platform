    ...
    def extract_output(self, accumulator):
        ...
        return {
            ...
            'avg_order_value': (
                ...
                if accumulator['total_orders'] > 0 else 0
            ),
...
def run_pipeline(project_id, runner='DirectRunner', pipeline_args=None):
    ...
    with beam.Pipeline(options=options) as pipeline:
        ...
        orders = (
            pipeline
            | 'Read Orders from BigQuery' >> beam.io.ReadFromBigQuery(
                query=orders_query,
        ...
        payments = (
            pipeline
            | 'Read Payments from BigQuery' >> beam.io.ReadFromBigQuery(
                query=payments_query,
                use_standard_sql=True
            )
            | 'Key Payments by Order' >> beam.Map(
                lambda p: (p['order_id'], p)
            )
        )
        ...
        enriched_orders = (
            orders
            | 'Enrich with Payment' >> beam.ParDo(
                EnrichWithPayment(),
        ...
        monthly_metrics = (
            enriched_orders
            | 'Key by Year-Month' >> beam.Map(
                lambda order: (
                    f"{order['year']}-{order['month']:02d}",
                    order
                )
            )
            | 'Compute Metrics' >> beam.CombinePerKey(
