...
class TestBeamParsers(unittest.TestCase):
    ...
    def test_parse_order_csv_valid(self):
        ...
        with TestPipeline() as p:
            output = p | beam.Create(input_data) | beam.ParDo(ParseOrder())
...
class TestBeamTransformations(unittest.TestCase):
    ...
    def test_enrichment_with_payment(self):
        ...
        payments = {"ord123": {"payment_type": "credit_card", "payment_value": 150.00}}

        with TestPipeline() as p:
            payments_pcoll = p | "Create Payments" >> beam.Create(
                [("ord123", payments["ord123"])]
            )
            ...
            output = (
                ...
                | "Enrich"
                >> beam.ParDo(EnrichWithPayment(), payments_dict=payments_dict)
            )
